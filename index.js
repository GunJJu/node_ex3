...
app.post("/posts", (req, res)=>{
    try {
        ...
        if(typeof subject !== 'string' || subject.trim()=='' || typeof desc !== 'string' || desc.trim()==''){
            return res.status(400).json({message:"subject, desc는 비어 있지 않은 문자열 이어야 합니다."})
        }
...
app.get('/posts',(req, res)=>{
    try {
        ...
        res.status(201).json({message:"게시글 1개 불러오기 완료", posts})
    } catch (error) {
...
app.get('/posts/:id',(req, res)=>{
    try {
        ...
        if(index === -1) {
            return res.status(400).json({message:"게시글 불러오기 실패"})
        }
        res.status(201).json({message:"게시글 1개 불러오기 완료", post: posts[index]})

    } catch (error) {
        console.log("게시글 1개 조회 중 오류", error)
        res.status(500).json({message:"서버 내부 오류 발생..!"})
    }
})
...
app.put('/posts/:id',(req, res)=>{
    try {
        ...
        return res.status(201).json({message:"게시글 1개 수정하기 완료", post: posts[index]})

    } catch (error) {
        console.log("게시글 1개 수정 중 오류", error)
        res.status(500).json({message:"서버 내부 오류 발생..!"})
    }
})
